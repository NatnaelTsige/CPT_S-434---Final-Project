...
class ConvNet(nn.Module):
    ...
    def forward(self, x):
        out = self.layer1(x)
        out = self.layer2(out)
        out = out.view(out.size(0), -1)  # flatten the output of the second convolutional layer
        out = self.fc(out)
        return out

    def _get_fc_input_size(self):
        # Pass a dummy tensor through the convolutional layers and print its shape
